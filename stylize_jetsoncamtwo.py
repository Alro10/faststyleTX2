...
if __name__ == '__main__':
    ...
    cap = cv2.VideoCapture("nvcamerasrc ! video/x-raw(memory:NVMM), width=(int)500, height=(int)500,format=(string)I420, framerate=(fraction)5/1 ! nvvidconv flip-method=0 ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink")
    ...
    with tf.Session() as sess:
        ...
        while(True):
            ...
            # Add this line to show original frame 
            numpy_horizontal = np.hstack((frame, img_out))
            numpy_horizontal_concat = np.concatenate((frame, img_out), axis=1)
